...
config = open('config_coinbase.json')
...
def queryKlines(symbol, granularity, start, end):
    """"""
    path = 'https://api.pro.coinbase.com/products/%s/candles'%symbol
    params = {
...
if __name__ == '__main__':
    ...
    for start, end in daterange(startDt, endDt, d):
        ...
        success, val = queryKlines(product, '300', start.isoformat(), end.isoformat())
        if success:
            for d in val:
                bar = VtBarData()
                bar.gatewayName = l[0]
                ...
                bar.datetime = datetime.datetime.utcfromtimestamp(d[0])
